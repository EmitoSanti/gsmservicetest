...
export class UsersService {
    static currentUser(id: any): Promise<IUser> {
        ...
        return Promise.resolve(User.findById({id: id}));
    }
    ...
    static authenticate(pass: string): Boolean {
        const password = pbkdf2Sync(pass, conf.passwordSalt, 10000, 64, "SHA1").toString("base64");
        return (pass && pass === password);
    }

    static validateLogin(body: SignInRequest): Promise<SignInRequest> {
        ...
        if (!UsersService.authenticate(body.password)) {
    ...
    static register(body: SignUpRequest): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            UsersService.validateRegister(body)
                .then(body => {
    ...
    static async changePassword(userId: string, body: ChangePasswordRequest): Promise<void> {
        try {
            let user = await UsersService.validateChangePassword(userId, body);
    ...
    static async validateChangePassword(userId: string, body: ChangePasswordRequest): Promise<IUser> {
        try {
            const result: error.ValidationErrorMessage = {
                messages: []
            };
    ...
    static async grant(userId: string, permissions: string[]): Promise<void> {
        try {
    ...
    static async revoke(userId: string, permissions: string[]): Promise<void> {
        try {
            if (!permissions || !(permissions instanceof Array)) {
                throw error.newArgumentError("permissions", "Invalid value");
            }
